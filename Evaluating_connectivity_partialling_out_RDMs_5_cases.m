...
for subject=1:100
    ...
    for numb=1:2
        for time=1:max(samples_sizes)
            ...
            elseif numb==2 % desired RDM
                ...
                for i=1:num_conditions
                    for j=i+1:num_conditions
                        if strcmp(desired_task{i},desired_task{j})==1 && strcmp(desired_semantics{i},desired_semantics{j})==1
                            X(i,j)=1;
                        elseif strcmp(desired_task{i},desired_task{j})==1 && strcmp(desired_semantics{i},desired_semantics{j})~=1
                            X(i,j)=0.5;
                        elseif strcmp(desired_task{i},desired_task{j})~=1 && strcmp(desired_semantics{i},desired_semantics{j})==1
                            X(i,j)=0.5;
                        else
    ...
    for case_number=1:5 %% 1=corr to task+semantics no partialling out; 2=corr to task nothing out;
        ...
        elseif case_number==4
            ...
            for t=delay+1:size(RDM_R_source,2)
                ...
                Front(t)= corr(RDM_R_destin(:,t),Model_RDM_task,'row','complete');
                Front_minus_back(t)= partialcorr(RDM_R_destin(:,t),Model_RDM_task,[nanmean(RDM_R_source(:,t-delay:t),2),Model_RDM_semantics],'row','complete');
                FF(t)=Front(t)-Front_minus_back(t);
                
                Back(t)= corr(RDM_R_source(:,t),Model_RDM_task,'row','complete');
                Back_minus_front(t)= partialcorr(RDM_R_source(:,t),Model_RDM_task,[nanmean(RDM_R_destin(:,t-delay:t),2),Model_RDM_semantics],'row','complete');
                FB(t)=Back(t)-Back_minus_front(t);
            ...
            for t=delay+1:size(RDM_R_source,2)
                for iteration=1:iterations
                    ...
                    Front_iter= corr(RDM_R_destin(:,t),randsample(Model_RDM_task,M),'row','complete');
                    Front_minus_back_iter= partialcorr(RDM_R_destin(:,t),randsample(Model_RDM_task,M),[nanmean(RDM_R_source(:,t-delay:t),2),Model_RDM_semantics],'row','complete');
                    FF_iter(t,iteration)=Front_iter-Front_minus_back_iter;
                    
                    Back_iter= corr(RDM_R_source(:,t),randsample(Model_RDM_task,M),'row','complete');
                    Back_minus_front_iter= partialcorr(RDM_R_source(:,t),randsample(Model_RDM_task,M),[nanmean(RDM_R_destin(:,t-delay:t),2),Model_RDM_semantics],'row','complete');
                    FB_iter(t,iteration)=Back_iter-Back_minus_front_iter;
        ...
        elseif case_number==5
            ...
            for t=delay+1:size(RDM_R_source,2)
                ...
                Front(t)= corr(RDM_R_destin(:,t),Model_RDM_semantics,'row','complete');
                Front_minus_back(t)= partialcorr(RDM_R_destin(:,t),Model_RDM_semantics,[nanmean(RDM_R_source(:,t-delay:t),2),Model_RDM_task],'row','complete');
                FF(t)=Front(t)-Front_minus_back(t);
                
                Back(t)= corr(RDM_R_source(:,t),Model_RDM_semantics,'row','complete');
                Back_minus_front(t)= partialcorr(RDM_R_source(:,t),Model_RDM_semantics,[nanmean(RDM_R_destin(:,t-delay:t),2),Model_RDM_task],'row','complete');
                FB(t)=Back(t)-Back_minus_front(t);
            ...
            for t=delay+1:size(RDM_R_source,2)
                for iteration=1:iterations
                    ...
                    Front_iter= corr(RDM_R_destin(:,t),randsample(Model_RDM_semantics,M),'row','complete');
                    Front_minus_back_iter= partialcorr(RDM_R_destin(:,t),randsample(Model_RDM_semantics,M),[nanmean(RDM_R_source(:,t-delay:t),2),Model_RDM_task],'row','complete');
                    FF_iter(t,iteration)=Front_iter-Front_minus_back_iter;
                    
                    Back_iter= corr(RDM_R_source(:,t),randsample(Model_RDM_semantics,M),'row','complete');
                    Back_minus_front_iter= partialcorr(RDM_R_source(:,t),randsample(Model_RDM_semantics,M),[nanmean(RDM_R_destin(:,t-delay:t),2),Model_RDM_task],'row','complete');
                    FB_iter(t,iteration)=Back_iter-Back_minus_front_iter;
        ...
        % 3=corr to task nothing out; 4=corr to task partialling out semantics; 5= corr to semantics partialling out task
            ...
            elseif case_number==3
                ...
                Back (t)= corr(RDM_R_source(:,t),Model_RDM_semantics,'row','complete');
            elseif case_number==4
                Front (t)= partialcorr(RDM_R_destin(:,t),Model_RDM_task,Model_RDM_semantics,'row','complete');
                Back (t)= partialcorr(RDM_R_source(:,t),Model_RDM_task,Model_RDM_semantics,'row','complete');
            elseif case_number==5
                Front (t)= partialcorr(RDM_R_destin(:,t),Model_RDM_semantics,Model_RDM_task,'row','complete');
                Back (t)= partialcorr(RDM_R_source(:,t),Model_RDM_semantics,Model_RDM_task,'row','complete');
            end
        end
    ...
    save(['Evaluating_connectivity_double_info_RDMs_5_casesT_C.mat'],'MVGC','FFs_E','FBs_E','FFs_iter_E','FBs_iter_E','FFs_H','FBs_H','FFs_iter_H','FBs_iter_H','Corr_to_Model')
end
ccc
...
load(['Evaluating_connectivity_double_info_RDMs_5_casesT_C.mat'],'MVGC','FFs_E','FBs_E','FFs_iter_E','FBs_iter_E','FFs_H','FBs_H','FFs_iter_H','FBs_iter_H','Corr_to_Model');
...
legend([Diffr{1}.mainLine Diffr{2}.mainLine Diffr{3}.mainLine Diffr{4}.mainLine Diffr{5}.mainLine],{'Both nothing out','Task nothing out','Semantics nothing out','Task Semantics out','Semantics Task out'})
set(gca,'fontsize', 18);
