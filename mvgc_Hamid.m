...
function [sig]=mvgc_Hamid(X,alpha,plotting)
...
mordert    = '';  % model order to use ('actual', 'AIC', 'BIC' or supplied numerical value)
momax     = 8;     % maximum model order for model order estimation

acmaxlags = [];   % maximum autocovariance lags (empty for automatic calculation)

tstat     = '';     % statistical test for MVGC:  'F' for Granger's F-test (default) or 'chi2' for Geweke's chi2 test
alpha     = alpha;   % significance level for significance test
...
%
% AT = var5_test;
% % AT(:,:,1)=[0 0 0;0 0 0.11;0 0 0.3];
% % AT(:,:,2)=[0 0 0.5;0 0 0.24;0 0 0.3];
% % AT(:,:,3)=[0 0 -0.2;0 0 0.22;0 0 0.35];
%
...
[A,SIG] = tsdata_to_var(X,mordert,regmode);
if plotting
    ptoc;
end
...
pval = mvgc_pval(F,mordert,nobs,ntrials,1,1,nvars-2,tstat); % take careful note of arguments!
