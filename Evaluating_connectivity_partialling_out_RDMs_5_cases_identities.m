...
MVGC=nan*ones(2,2,Num_subjects,5);
FFs_E=nan*ones(760,Num_subjects);
FBs_E=nan*ones(760,Num_subjects);
FFs_H=nan*ones(760,Num_subjects,5);
FBs_H=nan*ones(760,Num_subjects,5);
FFs_iter_E=nan*ones(760,iterations,Num_subjects);
FBs_iter_E=nan*ones(760,iterations,Num_subjects);
FFs_iter_H=nan*ones(760,iterations,Num_subjects,5);
FBs_iter_H=nan*ones(760,iterations,Num_subjects,5);
...
%                 desired_identities={'A','A','I','I','A','A','I','I','A','A','I','I','A','A','I','I'};
                ...
                for i=1:num_conditions
                    for j=i+1:num_conditions
                        if strcmp(desired_task{i},desired_task{j})==1 && strcmp(desired_identities{i},desired_identities{j})==1
                            X(i,j)=1;
                        elseif strcmp(desired_task{i},desired_task{j})==1 && strcmp(desired_identities{i},desired_identities{j})~=1
                            X(i,j)=0.5;
                        elseif strcmp(desired_task{i},desired_task{j})~=1 && strcmp(desired_identities{i},desired_identities{j})==1
                            X(i,j)=0.5;
                        else
                ...
                if time==1
                    ...
                    Model_RDM_identities=X_semantics(~isnan(X_semantics));
                end
            end
        end
    ...
    clearvars x data
    for t=1:722
        Corr_to_Model(subject,t,1)=corr(Model_RDM_both,RDM_R_source(:,t));
        Corr_to_Model(subject,t,2)=corr(Model_RDM_task,RDM_R_source(:,t));
        Corr_to_Model(subject,t,3)=corr(Model_RDM_identities,RDM_R_source(:,t));
        Corr_to_Model(subject,t,4)=corr(Model_RDM_both,RDM_R_destin(:,t));
        Corr_to_Model(subject,t,5)=corr(Model_RDM_task,RDM_R_destin(:,t));
        Corr_to_Model(subject,t,6)=corr(Model_RDM_identities,RDM_R_destin(:,t));
    end
    ...
    %
    %% Hamid's
    for case_number=1:5 %% 1=corr to task+semantics no partialling out; 2=corr to task nothing out;
        % 3=corr to task nothing out; 4=corr to task partialling out semantics; 5= corr to semantics partialling out task
        ...
        if case_number==1
            ...
            for t=delay+1:size(RDM_R_source,2)
                ...
                Front(t)= corr(RDM_R_destin(:,t),Model_RDM_both,'row','complete');
                Front_minus_back(t)= partialcorr(RDM_R_destin(:,t),Model_RDM_both,nanmean(RDM_R_source(:,t-delay:t),2),'row','complete');
                FF(t)=Front(t)-Front_minus_back(t);
                
                Back(t)= corr(RDM_R_source(:,t),Model_RDM_both,'row','complete');
                Back_minus_front(t)= partialcorr(RDM_R_source(:,t),Model_RDM_both,nanmean(RDM_R_destin(:,t-delay:t),2),'row','complete');
            ...
            for t=delay+1:size(RDM_R_source,2)
                for iteration=1:iterations
                    ...
                    Front_iter= corr(RDM_R_destin(:,t),randsample(Model_RDM_both,M),'row','complete');
                    Front_minus_back_iter= partialcorr(RDM_R_destin(:,t),randsample(Model_RDM_both,M),nanmean(RDM_R_source(:,t-delay:t),2),'row','complete');
                    FF_iter(t,iteration)=Front_iter-Front_minus_back_iter;
                    
                    Back_iter= corr(RDM_R_source(:,t),randsample(Model_RDM_both,M),'row','complete');
                    Back_minus_front_iter= partialcorr(RDM_R_source(:,t),randsample(Model_RDM_both,M),nanmean(RDM_R_destin(:,t-delay:t),2),'row','complete');
        ...
        elseif case_number==4
            ...
            for t=delay+1:size(RDM_R_source,2)
                ...
                Front(t)= corr(RDM_R_destin(:,t),Model_RDM_task,'row','complete');
                Front_minus_back(t)= partialcorr(RDM_R_destin(:,t),Model_RDM_task,[nanmean(RDM_R_source(:,t-delay:t),2),Model_RDM_identities],'row','complete');
                FF(t)=Front(t)-Front_minus_back(t);
                
                Back(t)= corr(RDM_R_source(:,t),Model_RDM_task,'row','complete');
                Back_minus_front(t)= partialcorr(RDM_R_source(:,t),Model_RDM_task,[nanmean(RDM_R_destin(:,t-delay:t),2),Model_RDM_identities],'row','complete');
                FB(t)=Back(t)-Back_minus_front(t);
            ...
            for t=delay+1:size(RDM_R_source,2)
                for iteration=1:iterations
                    ...
                    Front_iter= corr(RDM_R_destin(:,t),randsample(Model_RDM_task,M),'row','complete');
                    Front_minus_back_iter= partialcorr(RDM_R_destin(:,t),randsample(Model_RDM_task,M),[nanmean(RDM_R_source(:,t-delay:t),2),Model_RDM_identities],'row','complete');
                    FF_iter(t,iteration)=Front_iter-Front_minus_back_iter;
                    
                    Back_iter= corr(RDM_R_source(:,t),randsample(Model_RDM_task,M),'row','complete');
                    Back_minus_front_iter= partialcorr(RDM_R_source(:,t),randsample(Model_RDM_task,M),[nanmean(RDM_R_destin(:,t-delay:t),2),Model_RDM_identities],'row','complete');
                    FB_iter(t,iteration)=Back_iter-Back_minus_front_iter;
        ...
        elseif case_number==5
            ...
            for t=delay+1:size(RDM_R_source,2)
                ...
                Front(t)= corr(RDM_R_destin(:,t),Model_RDM_identities,'row','complete');
                Front_minus_back(t)= partialcorr(RDM_R_destin(:,t),Model_RDM_identities,[nanmean(RDM_R_source(:,t-delay:t),2),Model_RDM_task],'row','complete');
                FF(t)=Front(t)-Front_minus_back(t);
                
                Back(t)= corr(RDM_R_source(:,t),Model_RDM_identities,'row','complete');
                Back_minus_front(t)= partialcorr(RDM_R_source(:,t),Model_RDM_identities,[nanmean(RDM_R_destin(:,t-delay:t),2),Model_RDM_task],'row','complete');
                FB(t)=Back(t)-Back_minus_front(t);
            ...
            for t=delay+1:size(RDM_R_source,2)
                for iteration=1:iterations
                    ...
                    Front_iter= corr(RDM_R_destin(:,t),randsample(Model_RDM_identities,M),'row','complete');
                    Front_minus_back_iter= partialcorr(RDM_R_destin(:,t),randsample(Model_RDM_identities,M),[nanmean(RDM_R_source(:,t-delay:t),2),Model_RDM_task],'row','complete');
                    FF_iter(t,iteration)=Front_iter-Front_minus_back_iter;
                    
                    Back_iter= corr(RDM_R_source(:,t),randsample(Model_RDM_identities,M),'row','complete');
                    Back_minus_front_iter= partialcorr(RDM_R_source(:,t),randsample(Model_RDM_identities,M),[nanmean(RDM_R_destin(:,t-delay:t),2),Model_RDM_task],'row','complete');
                    FB_iter(t,iteration)=Back_iter-Back_minus_front_iter;
        ...
        for t=1:size(RDM_R_source,2) %% 1=corr to task+semantics no partialling out; 2=corr to task nothing out;
        % 3=corr to task nothing out; 4=corr to task partialling out semantics; 5= corr to semantics partialling out task
            if case_number==1
                Front (t)= corr(RDM_R_destin(:,t),Model_RDM_both,'row','complete');
                Back (t)= corr(RDM_R_source(:,t),Model_RDM_both,'row','complete');
            ...
            elseif case_number==3
                ...
                Back (t)= corr(RDM_R_source(:,t),Model_RDM_identities,'row','complete');
            elseif case_number==4
                Front (t)= partialcorr(RDM_R_destin(:,t),Model_RDM_task,Model_RDM_identities,'row','complete');
                Back (t)= partialcorr(RDM_R_source(:,t),Model_RDM_task,Model_RDM_identities,'row','complete');
            elseif case_number==5
                Front (t)= partialcorr(RDM_R_destin(:,t),Model_RDM_identities,Model_RDM_task,'row','complete');
                Back (t)= partialcorr(RDM_R_source(:,t),Model_RDM_identities,Model_RDM_task,'row','complete');
            end
        end
    ...
    save(['Evaluating_connectivity_double_info_RDMs_5_casesT_C_ident.mat'],'MVGC','FFs_E','FBs_E','FFs_iter_E','FBs_iter_E','FFs_H','FBs_H','FFs_iter_H','FBs_iter_H','Corr_to_Model')
end
ccc
...
% close all;
%
% load(['Evaluating_connectivity_double_info_RDMs_5_cases2.mat']);
% MVGC2=MVGC;
% FFs_E2=FFs_E;
% FBs_E2=FBs_E;
% FFs_iter_E2=FFs_iter_E;
% FBs_iter_E2=FBs_iter_E;
% FFs_H2=FFs_H;
% FBs_H2=FBs_H;
% FFs_iter_H2=FFs_iter_H;
% FBs_iter_H2=FBs_iter_H;
% Corr_to_Model2=Corr_to_Model;
%
% load(['Evaluating_connectivity_double_info_RDMs_5_cases.mat']);
% MVGC(:,:,51:end,:)=MVGC2(:,:,51:end,:);
% FFs_E(:,51:end,:)=FFs_E2(:,51:end,:);
% FBs_E(:,51:end,:)=FBs_E2(:,51:end,:);
% FFs_iter_E(:,:,51:end,:)=FFs_iter_E2(:,:,51:end,:);
% FBs_iter_E(:,:,51:end,:)=FBs_iter_E2(:,:,51:end,:);
% FFs_H(:,51:end,:)=FFs_H2(:,51:end,:);
% FBs_H(:,51:end,:)=FBs_H2(:,51:end,:);
% FFs_iter_H(:,:,51:end,:)=FFs_iter_H2(:,:,51:end,:);
% FBs_iter_H(:,:,51:end,:)=FBs_iter_H2(:,:,51:end,:);
% Corr_to_Model(51:100,:,:,:)=Corr_to_Model2(51:end,:,:,:);
%
% FFs_E(:,:,2:end)=[];
% FBs_E(:,:,2:end)=[];
% FFs_iter_E(:,:,:,2:end)=[];
...
% save(['Evaluating_connectivity_double_info_RDMs_5_casesT.mat'],'MVGC','FFs_E','FBs_E','FFs_iter_E','FBs_iter_E','FFs_H','FBs_H','FFs_iter_H','FBs_iter_H','Corr_to_Model');
load(['Evaluating_connectivity_double_info_RDMs_5_casesT_C_ident.mat'],'MVGC','FFs_E','FBs_E','FFs_iter_E','FBs_iter_E','FFs_H','FBs_H','FFs_iter_H','FBs_iter_H','Corr_to_Model');


method=3; % 1= Erin; 2=Hamid; 3=Alex
case_number=5; % 1=corr to task+semantics no partialling out; 2=corr to task nothing out;
        % 3=corr to identity nothing out; 4=corr to task partialling out identity; 5= corr to identity partialling out task
        
which_model=1; % 1=both , 2=task, 3=semantics
...
if method<3
    ...
    %     ylabel('Correlation to model RDM')
    %     ylim(ylims(1,:))
    %     xlim([1 700])
    %     set(gca,'fontsize', 18);
...
% case_number=5; %% 1=corr to task+semantics no partialling out; 2=corr to task+semantics partialling out task;
% 3=corr to task+semantics partialling out semantics; 4= corr to task and semantics out; 5= corr to semantics and task out
...
legend([Diffr{1}.mainLine Diffr{2}.mainLine Diffr{3}.mainLine Diffr{4}.mainLine Diffr{5}.mainLine],{'Both nothing out','Task nothing out','Identities nothing out','Task Identities out','Identities Task out'})
set(gca,'fontsize', 18);
