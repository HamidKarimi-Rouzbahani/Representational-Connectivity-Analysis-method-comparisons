...
FFs_E=nan*ones(760,Num_subjects,3);
FBs_E=nan*ones(760,Num_subjects,3);
FFs_H=nan*ones(760,Num_subjects,3);
FBs_H=nan*ones(760,Num_subjects,3);
FFs_iter_E=nan*ones(760,iterations,Num_subjects,3);
FBs_iter_E=nan*ones(760,iterations,Num_subjects,3);
FFs_iter_H=nan*ones(760,iterations,Num_subjects,3);
FBs_iter_H=nan*ones(760,iterations,Num_subjects,3);

for subject=1:Num_subjects
    for case_number=1:3 %% 1=every region related to its itself + FF; 2= every region related to its itself only; 3= only time-shifted_RDMS (totally similar)
        ...
        if case_number<3
            ...
            %     for time=1:size(RDM_random,2)
            %         cors_to_desired(time,1)=corr(squeeze(RDM_random(:,time)),Model_RDM_desired);
            %         cors_to_desired(time,2)=corr(squeeze(RDM_R1(:,time)),Model_RDM_desired);
            %     end
                ...
                for k=1:P
                    ...
                    for i=1:M*2
                        ...
                        elseif case_number==2
                            aloc(i,i)=abs(randn)*sigma;  %everything related to its past only
                        end
                    end
                    Arsig=[Arsig,aloc];
                end
            ...
            %         cors_to_desired(time,1)=corr(squeeze(data(1:M,time)),Model_RDM_desired);
            %         cors_to_desired(time,2)=corr(squeeze(data(M+1:end,time)),Model_RDM_desired);
            %     end
            %
            %     figure;
            %     plot(cors_to_desired)
            %             RDMs=data;
        ...
        for t=1:722
            Corr_to_Model(subject,case_number,t,1)=corr(Model_RDM_desired,RDM_R_source(:,t));
            Corr_to_Model(subject,case_number,t,2)=corr(Model_RDM_desired,RDM_R_destin(:,t));
        end
        ...
        FFs_E(1:size(FF,2),subject,case_number)=FF;
        FBs_E(1:size(FB,2),subject,case_number)=FB;
        FFs_iter_E(1:size(FF_iter,1),:,subject,case_number)=FF_iter;
        FBs_iter_E(1:size(FF_iter,1),:,subject,case_number)=FB_iter;
        ...
        for t=delay+1:size(RDM_R_source,2)
            ...
            Front(t)= corr(RDM_R_destin(:,t),Model_RDM_desired,'row','complete');
            Front_minus_back(t)= partialcorr(RDM_R_destin(:,t),Model_RDM_desired,nanmean(RDM_R_source(:,t-delay:t),2),'row','complete');
            FF(t)=Front(t)-Front_minus_back(t);
            
            Back(t)= corr(RDM_R_source(:,t),Model_RDM_desired,'row','complete');
            Back_minus_front(t)= partialcorr(RDM_R_source(:,t),Model_RDM_desired,nanmean(RDM_R_destin(:,t-delay:t),2),'row','complete');
        ...
        for t=delay+1:size(RDM_R_source,2)
            for iteration=1:iterations
                ...
                Front_iter= corr(RDM_R_destin(:,t),randsample(Model_RDM_desired,M),'row','complete');
                Front_minus_back_iter= partialcorr(RDM_R_destin(:,t),randsample(Model_RDM_desired,M),nanmean(RDM_R_source(:,t-delay:t),2),'row','complete');
                FF_iter(t,iteration)=Front_iter-Front_minus_back_iter;
                
                Back_iter= corr(RDM_R_source(:,t),randsample(Model_RDM_desired,M),'row','complete');
                Back_minus_front_iter= partialcorr(RDM_R_source(:,t),randsample(Model_RDM_desired,M),nanmean(RDM_R_destin(:,t-delay:t),2),'row','complete');
        ...
        for t=1:size(RDM_R_source,2)
            Front_task_semantics (t)= corr(RDM_R_destin(:,t),Model_RDM_desired,'row','complete');
            Back_task_semantics (t)= corr(RDM_R_source(:,t),Model_RDM_desired,'row','complete');
        end
        % figure;
        % plot([Front_task;Front_conditions;Back_task;Back_conditions]')
        significance_level=0.05;
        plotting=0;
        MVGC(:,:,subject,case_number)=mvgc_Hamid([Front_task_semantics;Back_task_semantics],significance_level,plotting);
        %     close all;
        [subject case_number]
    end
    save(['Evaluating_connectivity_shifted_RDMs_3_cases.mat'],'MVGC','FFs_E','FBs_E','FFs_iter_E','FBs_iter_E','FFs_H','FBs_H','FFs_iter_H','FBs_iter_H','Corr_to_Model')
end
...
% load(['Evaluating_connectivity_shifted_RDMs_3_cases_new_after34.mat']);
% MVGC2=MVGC;
% FFs_E2=FFs_E;
% FBs_E2=FBs_E;
...
%
% load(['Evaluating_connectivity_shifted_RDMs_3_cases_new.mat']);
% MVGC(:,:,35:end,:)=MVGC2(:,:,35:end,:);
% FFs_E(:,35:end,:)=FFs_E2(:,35:end,:);
% FBs_E(:,35:end,:)=FBs_E2(:,35:end,:);
% FFs_iter_E(:,:,35:end,:)=FFs_iter_E2(:,:,35:end,:);
% FBs_iter_E(:,:,35:end,:)=FBs_iter_E2(:,:,35:end,:);
% FFs_H(:,35:end,:)=FFs_H2(:,35:end,:);
% FBs_H(:,35:end,:)=FBs_H2(:,35:end,:);
% FFs_iter_H(:,:,35:end,:)=FFs_iter_H2(:,:,35:end,:);
% FBs_iter_H(:,:,35:end,:)=FBs_iter_H2(:,:,35:end,:);
% Corr_to_Model(35:100,:,:,:)=Corr_to_Model2(35:end,:,:,:);

% save(['Evaluating_connectivity_shifted_RDMs_3_cases_new_pooled.mat'],'MVGC','FFs_E','FBs_E','FFs_iter_E','FBs_iter_E','FFs_H','FBs_H','FFs_iter_H','FBs_iter_H','Corr_to_Model');
load(['Evaluating_connectivity_shifted_RDMs_3_cases.mat'],'MVGC','FFs_E','FBs_E','FFs_iter_E','FBs_iter_E','FFs_H','FBs_H','FFs_iter_H','FBs_iter_H','Corr_to_Model');
ccc

method=3; % 1= Erin; 2=Hamid; 3=Alex
case_number=1; %% 1=every region related to its itself + FF; 2= every region related to its itself only; 3= only time-shifted_RDMS (totally similar)
...
if method<3
    ...
    figure;
    subplot(3,1,1)
    source_cor=shadedErrorBar([1:size(Corr_to_Model,3)],squeeze(nanmean(Corr_to_Model(:,case_number,:,1))),squeeze(nanstd(Corr_to_Model(:,case_number,:,1)))./sqrt(subjects),{'color',[0.1 0.1 0.8],'LineWidth',2},1);
    hold on;
    destin_cor=shadedErrorBar([1:size(Corr_to_Model,3)],squeeze(nanmean(Corr_to_Model(:,case_number,:,2))),squeeze(nanstd(Corr_to_Model(:,case_number,:,2)))./sqrt(subjects),{'color',[0.8 0.1 0.1],'LineWidth',2},1);
    line([1 size(Corr_to_Model,3)],[0 0],'linestyle','--','color','k')
    ...
    imagesc(squeeze(nanmean(MVGC(:,:,:,case_number),3)),[0 1])
    xticks([1:4])
    xtickangle(45)
    ...
    ylabel('To')
    CB=colorbar;
%     set(get(CB,'title'),'string','Information flow');
